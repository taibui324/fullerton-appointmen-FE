...
export default function MyBooking() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5003/userbookings/search/" + email)
      .then((res) => {
        if (res.data == 0) {
          console.log("no data");
        } else {
  ...
  function clickDelete(id) {
    if (window.confirm("are you sure you want to delete the booking?")) {
      axios.delete("http://localhost:5003/userbookings/" + id).then((res) => {
        // console.log(res.data);
  ...
  function testSubmit() {
    ...
    axios.post("http://localhost:5003/usertests/add", testdata).then((res) => {
